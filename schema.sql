...
CREATE TABLE classrooms (
    ...
    CONSTRAINT chk_building CHECK (TRIM(building) <> '')
);
...
SELECT
    s.student_id,
    s.name,
    ROUND(SUM(g.grade_points * c.credits) / SUM(c.credits), 2) AS gpa
FROM students s
...
--    Prevents duplicate enrollment and returns a status message.
DELIMITER $$
...
--    Returns course title, semester, year, letter grade, and grade points.
DELIMITER $$
...
-- 1. Auto-set grade_points from letter_grade on insert.
...
--    Recalculates Pending / Partial / Paid every time a payment is inserted.
DELIMITER $$
...
BEGIN
    ...
    SELECT SUM(amount) INTO _paid FROM payments WHERE tuition_id = NEW.tuition_id;
    SELECT amount      INTO _due  FROM tuition   WHERE tuition_id = NEW.tuition_id;
